...
def get_joint_angles(user, tfBuffer, test, mirrored):
    ...
    if test:
        ...
        joint_angles['right']['right_w2'] = theta
    ...
    if mirrored:
        ...
        unmirrored['right']['right_w2'] = theta
        return unmirrored
...
def get_frame_positions(user, tfBuffer):
    ...
    try:
        for frame in FRAMES:
            transformation= tfBuffer.lookup_transform(BASE_FRAME, "%s_%d" % (frame, user), rospy.Time(),rospy.Duration(1.0))
...
def teleoperate(rate, user, test, mirrored):
    ...
    while not rospy.is_shutdown():
        rate.sleep()
        wii_listen()
#        listener.waitForTransform("/torso_1","camera_depth_frame",now,rospy.Duration(4.0))
